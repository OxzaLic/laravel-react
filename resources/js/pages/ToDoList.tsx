...
const App: React.FC = () => {
    ...
    const addList = () => {
        ...
        setLists([...lists, { id: Date.now(), name: newListName.trim(), todos: [] }]);
        setNewListName('');
    };
        ...
        setLists(
            lists.map((l) =>
                l.id === listId
                    ? {
                          ...l,
                          todos: [...l.todos, { id: Date.now(), text, dueDate, completed: false }],
                      }
                    : l,
            ),
        );
        ...
        setLists(
            lists.map((l) =>
                l.id === listId
                    ? {
                          ...l,
                          todos: l.todos.map((t) => (t.id === todoId ? { ...t, completed: !t.completed } : t)),
                      }
                    : l,
            ),
        );
    ...
    const deleteTodo = (listId: number, todoId: number) => {
        setLists(lists.map((l) => (l.id === listId ? { ...l, todos: l.todos.filter((t) => t.id !== todoId) } : l)));
    };
            ...
            setLists(
                lists.map((l) =>
                    l.id === listId
                        ? {
                              ...l,
                              todos: l.todos.map((t) => (t.id === todoId ? { ...t, text: newText.trim(), dueDate: newDue } : t)),
                          }
                        : l,
                ),
            );
    ...
    return (
        <div style={styles.container}>
            <h1 style={styles.mainHeading}>To-Do Lists</h1>
            ...
            {lists.map((list) => (
                <div key={list.id} style={styles.listContainer}>
                    <div style={styles.listHeader}>
                        ...
                        {/* เปลี่ยนตรงนี้เป็นปุ่ม Delete */}
                        <button onClick={() => deleteList(list.id)} style={styles.deleteListButton}>
                            Delete
                        </button>
                    </div>
                    ...
                    {list.todos.map((todo) => (
                        ...
                        >
                            <span style={styles.icon} onClick={() => toggleTodo(list.id, todo.id)}>
                                {todo.completed ? '✅' : '⭕'}
                            </span>
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={styles.due}>Due: {formatDate(todo.dueDate)}</div>
                            </div>

                            <button onClick={() => deleteTodo(list.id, todo.id)} style={styles.deleteButton}>
                                🗑️
                            </button>
                            <button onClick={() => editTodo(list.id, todo.id)} style={styles.editButton}>
                                ✏️
                            </button>
                        </div>
                    ))}
...
const AddTodoForm: React.FC<{
    ...
    return (
        <div style={styles.newTaskBox}>
            ...
            <input type="text" placeholder="Create a new task" value={text} onChange={(e) => setText(e.target.value)} style={styles.input} />
            <input type="date" value={due} onChange={(e) => setDue(e.target.value)} style={styles.dateInput} />
